...
vec3 CalcDirLight(Light light, vec3 normal, vec3 fragPos, float depth, vec3 viewDir, vec3 albedo, vec3 specIntensity, float specPower, vec3 ambient);
vec3 CalcPointLight(Light light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 albedo, vec3 specIntensity, float specPower, vec3 ambient);
vec3 CalcSpotLight(Light light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 albedo, vec3 specIntensity, float specPower, vec3 ambient);
float CalcShadowPCF(vec4 fragPosLightSpace, vec3 normal, vec3 lightDir, int shadowMapIndex);

void main()
{
	vec3 norm = texture(gNormal, fs_in.TexCoord).xyz;
	...
	for( int i=0; i <  numLight; i++ )
	{
		...
		{
			result += CalcDirLight(lights[i], norm, fragPos, depth, viewDir, albedo, specIntensity, specPower, ambient);		
		}
        else if ( lights[i].type == 1 ) // Point Light
		{
			result += CalcPointLight(lights[i], norm, fragPos, viewDir, albedo, specIntensity, specPower, ambient);
		}
        else if( lights[i].type == 2 ) // Spot Light
		{
			result += CalcSpotLight(lights[i], norm, fragPos, viewDir, albedo, specIntensity, specPower, ambient);
		}
	}
...
vec3 CalcDirLight(Light light, vec3 normal, vec3 fragPos, float depth, vec3 viewDir, vec3 albedo, vec3 specIntensity, float specPower, vec3 ambientCoef)
{
	...
	{
		shadow = max( shadow, CalcShadowPCF(light.viewProj * vec4(fragPos, 1.0), normal, lightDir, light.shadowMapIndices[i]));
	}

    float vsCamDepth = depth;
	...
	if(light.cascadeShadowIndices[0] != -1)
	{
		...
		shadow = max( shadow, CalcShadowPCF(cascadeShadowData[cascadeIndex].viewProj * vec4(fragPos, 1.0), normal, lightDir, cascadeShadowData[cascadeIndex].shadowMapIndex));
	}

	return ambient + (1.0 - shadow) * (diffuse + specular);
}

vec3 CalcPointLight(Light light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 albedo, vec3 specIntensity, float specPower, vec3 ambientCoef)
{
...
vec3 CalcSpotLight(Light light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 albedo, vec3 specIntensity, float specPower, vec3 ambientCoef)
{
	vec3 lightDir = normalize(light.position - fragPos);
	// diffuse shading
	float diff = max( dot(normal, lightDir), 0.0);
